    ...
    stage("Check app is available") {
      steps {
        script {
          ...
          def appUrl = sh(returnStdout: true, script: """curl --connect-timeout 5 --max-time 10 --retry 5 --retry-delay 0 --retry-max-time 40 'http://${env.INSTANCE_IP}:8080/helloworld/'""")
