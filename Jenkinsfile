    ...
    stage("Check app is available") {
      steps {
        script {
          ...
          def appUrl = sh(returnStdout: true, script: """curl --connect-timeout 10 --max-time 20 --retry 10 --retry-delay 0 --retry-max-time 120 'http://${env.INSTANCE_IP}:8080/helloworld/'""")
